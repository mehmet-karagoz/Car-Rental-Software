...
<body>
    ...
    $customerRentSql = "SELECT * FROM rent WHERE customer_id=" . $customerId;
    $result = $link->query($customerRentSql);
    while ($row = mysqli_fetch_assoc($result)) {

        $pickUpAddress = $row["pick_up_address"];
        $returnAddress = $row["return_address"];

        $carSql = "SELECT * FROM carbooking WHERE rent_id=" . $row["rent_id"];
        $carResult = $link->query($carSql);
        $carRow = mysqli_fetch_assoc($carResult);
        $startDate = $carRow["start_date"];
        $endDate = $carRow["end_date"];

        $carModelSql = "SELECT * FROM car WHERE car_id=" . $carRow["car_id"];
